...
function setup(){
	...
	button_colors = [color(0, 255, 0), color(255, 255, 0), color(0, 255, 255),
					color(0, 100, 255), color(255, 127, 0), color(100, 0, 150)];
...
function toggle_ld_edit(){
	if((seq[0].editor.last_ed_change + 100) < millis()){
	...
	if (global_opt.value() == 'Rand All'){				// Randomize all sequencer buttons
		for(var i = 0; i < seq.length; i++){
			for(var j = 0; j < seq[i].buttons.length; j++){
				...
				if(state == true) {
					seq[i].buttons[j].t_counter = 1;
					seq[i].idle_buttons[j].t_counter = 1;
				} else {
					seq[i].buttons[j].t_counter = 0;
					seq[i].idle_buttons[j].t_counter = 0;
				}
...
function Lead(){
	...
	this.filt2 = new p5.LowPass();
	this.filt2.res = 0.5;
	
	this.env = new p5.Env(0.01, 1, 1, 0.5);
	this.env.setRange(.6, 0);
	this.env2 = new p5.Env(0.01, 1, 1, 0.5);
	this.env2.setRange(.6, 0);

	this.filt_env = new p5.Env(.25, .1, .75, .1);
	this.filt_env.setRange(2000, 0);
	this.filt.freq(this.filt_env);

	this.filt_env2 = new p5.Env(.25, .1, .75, .1);
	this.filt_env2.setRange(2000, 0);
	this.filt2.freq(this.filt_env2);

	this.osc.disconnect();
	this.osc2.disconnect();
	this.osc.connect(this.filt);
	this.osc2.connect(this.filt2);
	...
	this.play = function(){
		...
		this.osc.freq(midiToFreq(this.note));
		this.osc2.freq(midiToFreq(this.note) + 1);
		this.filt_env.play();
		this.filt_env2.play();
		this.env.play(this.osc);
